...
if __name__ == "__main__":
    in_dir = "/cbica/projects/executive_function/EF_dataset/derivatives/qsirecon_unzipped/qsirecon/derivatives"
    ...
    for title, pattern in patterns.items():
        # Debugging
        raw_maps = sorted(glob(os.path.join(in_dir, pattern)))
        print(f"[DEBUG] {title} raw matches (before exclusions): {len(raw_maps)}")
        print("[DEBUG] Example paths:", raw_maps[:5])
