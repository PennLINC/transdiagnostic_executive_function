---
title: "diagnostic_distribution.Rmd"
output: html_document
date: "2025-07-10"
---

This script summarizes diagnostic information and creates summary tables and sankey plot, to visualize clinical diagnostic phenotypes.

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r load packages, include=FALSE}
library(tidyverse)
library(dplyr)
library(networkD3)
library(webshot2)
library(webshot)
```

```{r load in csvs}
# Load in final clinical diagnosis df (cleaned up from clinical_data_cleaning.Rmd script)
clinical_diagnosis_df <- read.csv('/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/UPDATED_clinical_scanned_clean.csv') # local path

```


```{r separate out csvs}
ADHD_df <- clinical_diagnosis_df %>% filter(study_group == "ADHD")
TDNC_df <- clinical_diagnosis_df %>% filter(study_group == "TD/NC")
PROCHR_df <- clinical_diagnosis_df %>% filter(study_group == "PRO/CHR")

# Calculate counts and percentages
study_group_summary <- clinical_diagnosis_df %>%
  group_by(study_group) %>%
  summarise(
    count = n()
  ) %>%
  mutate(
    percentage = round(100 * count / sum(count), 1)
  )

# Print the result
print(study_group_summary)
```

NOTE: dx_sum is inaccurate for healthy controls: counts dx_none as a diagnosis in the csv file!

```{r number of diagnoses by group}
# Calculate average dxsum (number of diagnoses) per study_group
dxsum_avg_summary <- clinical_diagnosis_df %>%
  filter(study_group %in% c("ADHD", "TD/NC", "PRO/CHR")) %>%  # Optional: ensure group validity
  mutate(dxsum = as.numeric(dxsum)) %>%  # Ensure dxsum is numeric
  group_by(study_group) %>%
  summarise(avg_dxsum = mean(dxsum, na.rm = TRUE)) %>%
  ungroup()


dxsum_avg_summary

```
TD/NC will be wrong bc dx_none counts as 1, but should count as 0.


```{r summarize diagnoses}

# Identify dx_ columns
dx_cols <- grep("^dx_", names(clinical_diagnosis_df), value = TRUE)
dx_cols <- setdiff(dx_cols, "dx_pscat")  # Exclude dx_pscat

# Function to summarize dx_ counts for each group
summarize_dx <- function(df, group_name) {
  # Ensure dx_ columns are numeric
  dx_data <- df %>%
    select(all_of(dx_cols)) %>%
    mutate(across(everything(), ~ as.numeric(as.character(.))))
  
  # Sum across columns
  dx_sums <- colSums(dx_data, na.rm = TRUE)
  
  # Return as a row in a data frame
  data.frame(study_group = group_name, t(dx_sums), check.names = FALSE)
}

# Apply the function to each study group
ADHD_summary <- summarize_dx(ADHD_df, "ADHD")
TDNC_summary <- summarize_dx(TDNC_df, "TD/NC")
PROCHR_summary <- summarize_dx(PROCHR_df, "PRO/CHR")

# Combine into one summary data frame
summary_df <- bind_rows(ADHD_summary, TDNC_summary, PROCHR_summary)

# Exclude remitted ones from summary_df
summary_df <- summary_df %>%
  select(-dx_prodromal_remit, -dx_psychosis_remit, -dx_scz_remit, -dx_moodnos_remit, -dx_mdd_remit, -dx_bp_remit, -dx_adhd_remit, -dx_ptsd_remit, -dx_other_remit)


# View or export the result
print(summary_df)
#write.csv(summary_df, file = #"/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnoses_wideform.csv", row.names = FALSE) # local path

```
```{r distribution}
# Assuming `summary_df` is your 3-row summary table with counts
# Convert wide to long format
summary_long <- summary_df %>%
  pivot_longer(cols = starts_with("dx_"),
               names_to = "diagnosis",
               values_to = "count")

# Normalize to proportions within each study_group
summary_long <- summary_long %>%
  group_by(study_group) %>%
  mutate(prop = count / sum(count))

# Define labels for ggplot key (change this once you figure out what exact labels mean)
diagnosis_labels <- c(
  dx_adhd = "ADHD",
  dx_adhd_remit = "ADHD (Remitted)",
  dx_bp = "BPD",
  dx_bp_remit = "BPD (Remitted)",
  dx_mdd = "MDD",
  dx_mdd_remit = "MDD (Remitted)",
  dx_moodnos = "Mood Disorder NOS",
  dx_moodnos_remit = "Mood Disorder NOS (Remitted)",
  dx_none = "No Diagnosis",
  dx_other = "Other",
  dx_other_remit = "Other (Remitted)",
  dx_prodromal = "CHR/Prodromal Psychosis",
  dx_prodromal_remit = "CHR/Prodromal Psychosis (Remitted)",
  dx_psychosis = "Psychosis",
  dx_psychosis_remit = "Psychosis (Remitted)",
  dx_scz = "Schizophrenia",
  dx_scz_remit = "Schizophrenia (Remitted)",
  dx_ptsd = "PTSD",
  dx_ptsd_remit = "PTSD (Remitted)"
)
#Replace diagnosis values using new names
summary_long <- summary_long %>%
  mutate(diagnosis = recode(diagnosis, !!!diagnosis_labels))
summary_long #use this if you want to include in graph key/figure label label where count might = 0 (longer figure key/legend)
summary_long_no0 <- summary_long %>%
  filter(count != 0)
summary_long_no0 #use this if you only want graph to show labels where count does not = 0

```

```{r sankey plot with percentages - proportion based on percentage of unique participants in dataset who have that diagnosis out of total participants, and left side is based on percentage of unique participants with that study_group }

# Load packages
library(dplyr)
library(tidyr)
library(networkD3)
library(htmlwidgets)

# Ensure all TD/NC participants have dx_none == 1 - note that you need to do this manually bc some of them don't have dx_none == 1, likely bc they had a 1 for something not surveyed as part of EF!!
clinical_diagnosis_df <- clinical_diagnosis_df %>%
  mutate(dx_none = ifelse(study_group == "TD/NC", 1, dx_none))

# Step 1: Create clean source/target labels
summary_long_no0 <- summary_long_no0 %>%
  mutate(
    source_label = study_group,
    target_label = diagnosis
  )

# Step 2: Get total number of unique participants
total_participants <- clinical_diagnosis_df %>%
  distinct(bblid) %>%
  nrow()

# Step 3: Compute % of participants per study group (left side of Sankey)
source_flows <- clinical_diagnosis_df %>%
  group_by(source_label = study_group) %>%
  summarise(unique_participants = n_distinct(bblid), .groups = "drop") %>%
  mutate(
    source_pct = round(100 * unique_participants / total_participants, 1),
    source_label_pct = paste0(source_label, " (", source_pct, "%)")
  )

# Step 4: Compute % of participants per diagnosis (right side of Sankey)
dx_cols <- grep("^dx_", names(clinical_diagnosis_df), value = TRUE)
dx_cols <- setdiff(dx_cols, "dx_pscat")  # Exclude if necessary

long_dx <- clinical_diagnosis_df %>%
  select(bblid, all_of(dx_cols)) %>%
  pivot_longer(cols = all_of(dx_cols), names_to = "diagnosis", values_to = "has_dx") %>%
  filter(has_dx == 1)

target_flows <- long_dx %>%
  group_by(target_label = diagnosis) %>%
  summarise(unique_participants = n_distinct(bblid), .groups = "drop") %>%
  mutate(
    target_pct = round(100 * unique_participants / total_participants, 1)
  )

# Step 5: Apply readable labels to diagnosis names
diagnosis_labels <- c(
  dx_adhd = "ADHD",
  dx_adhd_remit = "ADHD (Remitted)",
  dx_bp = "BPD",
  dx_bp_remit = "BPD (Remitted)",
  dx_mdd = "MDD",
  dx_mdd_remit = "MDD (Remitted)",
  dx_moodnos = "Mood Disorder NOS",
  dx_moodnos_remit = "Mood Disorder NOS (Remitted)",
  dx_none = "No Diagnosis",
  dx_other = "Other",
  dx_other_remit = "Other (Remitted)",
  dx_prodromal = "CHR/Prodromal Psychosis",
  dx_prodromal_remit = "CHR/Prodromal Psychosis (Remitted)",
  dx_psychosis = "Psychosis",
  dx_psychosis_remit = "Psychosis (Remitted)",
  dx_scz = "Schizophrenia",
  dx_scz_remit = "Schizophrenia (Remitted)",
  dx_ptsd = "PTSD",
  dx_ptsd_remit = "PTSD (Remitted)"
)

target_flows <- target_flows %>%
  mutate(target_label = recode(target_label, !!!diagnosis_labels)) %>%
  mutate(target_label_pct = paste0(target_label, " (", target_pct, "%)"))

# Step 6: Join percent labels back to main summary
summary_long_labeled <- summary_long_no0 %>%
  left_join(source_flows, by = "source_label") %>%
  left_join(target_flows, by = "target_label")

# Step 7: Create nodes and links
nodes <- data.frame(name = unique(c(summary_long_labeled$source_label_pct,
                                    summary_long_labeled$target_label_pct)))

links <- summary_long_labeled %>%
  mutate(
    source = match(source_label_pct, nodes$name) - 1,
    target = match(target_label_pct, nodes$name) - 1
  ) %>%
  select(source, target, value = count)

# Step 8: Plot Sankey
sankey_plot <- sankeyNetwork(Links = links,
              Nodes = nodes,
              Source = "source",
              Target = "target",
              Value = "value",
              NodeID = "name",
              fontSize = 12,
              nodeWidth = 30)

# Step 9: Save HTML and PNG
saveWidget(sankey_plot,
           file = "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_2.html",
           selfcontained = TRUE)

webshot2::webshot(
  url = "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_2.html",
  file = "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_2.png",
  vwidth = 1000,
  vheight = 500,
  zoom = 3.83
) # local paths - correspond to 'clinical/clinical_figures/' + '/diagnosis_sankey_plot.html' and + '/diagnosis_sankey_plot.png' in github folder


```

``` {r sankey no labels pt 2}
# Load packages
library(dplyr)
library(tidyr)
library(networkD3)
library(htmlwidgets)
library(htmltools)
library(webshot2)

# Ensure all TD/NC participants have dx_none == 1
clinical_diagnosis_df <- clinical_diagnosis_df %>%
  mutate(dx_none = ifelse(study_group == "TD/NC", 1, dx_none))

# Step 1: Create clean source/target labels
summary_long_no0 <- summary_long_no0 %>%
  mutate(
    source_label = study_group,
    target_label = diagnosis
  )

# Step 2: Get total number of unique participants
total_participants <- clinical_diagnosis_df %>%
  distinct(bblid) %>%
  nrow()

# Step 3: Compute % of participants per study group (left side of Sankey)
source_flows <- clinical_diagnosis_df %>%
  group_by(source_label = study_group) %>%
  summarise(unique_participants = n_distinct(bblid), .groups = "drop") %>%
  mutate(
    source_pct = round(100 * unique_participants / total_participants, 1),
    source_label_pct = paste0(source_label, " (", source_pct, "%)")
  )

# Step 4: Compute % of participants per diagnosis (right side of Sankey)
dx_cols <- grep("^dx_", names(clinical_diagnosis_df), value = TRUE)
dx_cols <- setdiff(dx_cols, "dx_pscat")  # Exclude if necessary

long_dx <- clinical_diagnosis_df %>%
  select(bblid, all_of(dx_cols)) %>%
  pivot_longer(cols = all_of(dx_cols), names_to = "diagnosis", values_to = "has_dx") %>%
  filter(has_dx == 1)

target_flows <- long_dx %>%
  group_by(target_label = diagnosis) %>%
  summarise(unique_participants = n_distinct(bblid), .groups = "drop") %>%
  mutate(
    target_pct = round(100 * unique_participants / total_participants, 1)
  )

# Step 5: Apply readable labels to diagnosis names
diagnosis_labels <- c(
  dx_adhd = "ADHD",
  dx_adhd_remit = "ADHD (Remitted)",
  dx_bp = "BPD",
  dx_bp_remit = "BPD (Remitted)",
  dx_mdd = "MDD",
  dx_mdd_remit = "MDD (Remitted)",
  dx_moodnos = "Mood Disorder NOS",
  dx_moodnos_remit = "Mood Disorder NOS (Remitted)",
  dx_none = "No Diagnosis",
  dx_other = "Other",
  dx_other_remit = "Other (Remitted)",
  dx_prodromal = "CHR/Prodromal Psychosis",
  dx_prodromal_remit = "CHR/Prodromal Psychosis (Remitted)",
  dx_psychosis = "Psychosis",
  dx_psychosis_remit = "Psychosis (Remitted)",
  dx_scz = "Schizophrenia",
  dx_scz_remit = "Schizophrenia (Remitted)",
  dx_ptsd = "PTSD",
  dx_ptsd_remit = "PTSD (Remitted)"
)

target_flows <- target_flows %>%
  mutate(target_label = recode(target_label, !!!diagnosis_labels)) %>%
  mutate(target_label_pct = paste0(target_label, " (", target_pct, "%)"))

# Step 6: Join percent labels back to main summary
summary_long_labeled <- summary_long_no0 %>%
  left_join(source_flows, by = "source_label") %>%
  left_join(target_flows, by = "target_label")

# Step 7: Create nodes and links
nodes <- data.frame(
  name = unique(c(summary_long_labeled$source_label_pct,
                  summary_long_labeled$target_label_pct)),
  stringsAsFactors = FALSE
)

links <- summary_long_labeled %>%
  mutate(
    source = match(source_label_pct, nodes$name) - 1,
    target = match(target_label_pct, nodes$name) - 1
  ) %>%
  select(source, target, value = count)

# Step 8: Plot Sankey (identical), then hide in-graph labels
sankey_plot <- sankeyNetwork(
  Links = links,
  Nodes = nodes,
  Source = "source",
  Target = "target",
  Value  = "value",
  NodeID = "name",
  fontSize = 12,
  nodeWidth = 30
)

sankey_plot <- htmlwidgets::onRender(
  sankey_plot,
  jsCode = "
  function(el, x) {
    d3.select(el).selectAll('.node text').text(''); // blank node labels
  }
  "
)

# Step 9: Save Sankey HTML and PNG (same workflow as your original)
out_html <- "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_blank.html"
out_png  <- "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_blank.png"

htmlwidgets::saveWidget(
  sankey_plot,
  file = out_html,
  selfcontained = TRUE
)

webshot2::webshot(
  url     = out_html,
  file    = out_png,
  vwidth  = 1000,
  vheight = 500,
  zoom    = 3.83
)

# ---------- Separate Legend HTML (matching colors & percentages) ----------

# Recreate the exact node color mapping (Category20 cycling by node index)
d3_schemeCategory20 <- c(
  "#1f77b4","#aec7e8","#ff7f0e","#ffbb78","#2ca02c",
  "#98df8a","#d62728","#ff9896","#9467bd","#c5b0d5",
  "#8c564b","#c49c94","#e377c2","#f7b6d2","#7f7f7f",
  "#c7c7c7","#bcbd22","#dbdb8d","#17becf","#9edae5"
)
node_index  <- seq_len(nrow(nodes))
node_colors <- d3_schemeCategory20[((node_index - 1) %% length(d3_schemeCategory20)) + 1]
names(node_colors) <- nodes$name

# Legend labels taken from the same sources used to create nodes
left_labels  <- unique(summary_long_labeled$source_label_pct)
right_labels <- unique(summary_long_labeled$target_label_pct)

legend_css <- tags$style(HTML("
  .legend { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;
            max-width: 560px; padding: 12px 8px; }
  .legend h3 { margin: 0 0 8px 0; font-size: 16px; }
  .legend h4 { margin: 14px 0 8px 0; font-size: 14px; font-weight: 600; }
  .legend-item { display: flex; align-items: center; margin: 4px 0; font-size: 13px; }
  .legend-swatch { width: 14px; height: 14px; margin-right: 8px; border-radius: 3px;
                   border: 1px solid rgba(0,0,0,0.15); }
"))

legend_html <- tags$html(
  tags$head(tags$title("Diagnosis Sankey Legend")),
  tags$body(
    tags$div(
      class = "legend",
      legend_css,
      tags$h3("Legend"),
      tags$h4("Study Groups (share of all participants)"),
      lapply(left_labels, function(lbl) {
        col <- unname(node_colors[lbl])
        tags$div(class = "legend-item",
                 tags$span(class = "legend-swatch", style = paste0("background:", col, ";")),
                 tags$span(lbl))
      }),
      tags$h4("Diagnoses (share of all participants)"),
      lapply(right_labels, function(lbl) {
        col <- unname(node_colors[lbl])
        tags$div(class = "legend-item",
                 tags$span(class = "legend-swatch", style = paste0("background:", col, ";")),
                 tags$span(lbl))
      })
    )
  )
)

legend_file <- "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_legend.html"
htmltools::save_html(legend_html, file = legend_file, background = "white")

```
``` {r sankey no labels pt 3}
# Load packages
library(dplyr)
library(tidyr)
library(networkD3)
library(htmlwidgets)
library(htmltools)
library(webshot2)

# Ensure all TD/NC participants have dx_none == 1
clinical_diagnosis_df <- clinical_diagnosis_df %>%
  mutate(dx_none = ifelse(study_group == "TD/NC", 1, dx_none))

# Step 1: Create clean source/target labels
summary_long_no0 <- summary_long_no0 %>%
  mutate(
    source_label = study_group,
    target_label = diagnosis
  )

# Step 2: Get total number of unique participants
total_participants <- clinical_diagnosis_df %>%
  distinct(bblid) %>%
  nrow()

# Step 3: Compute % of participants per study group (left side of Sankey)
source_flows <- clinical_diagnosis_df %>%
  group_by(source_label = study_group) %>%
  summarise(unique_participants = n_distinct(bblid), .groups = "drop") %>%
  mutate(
    source_pct = round(100 * unique_participants / total_participants, 1),
    source_label_pct = paste0(source_label, " (", source_pct, "%)")
  )

# Step 4: Compute % of participants per diagnosis (right side of Sankey)
dx_cols <- grep("^dx_", names(clinical_diagnosis_df), value = TRUE)
dx_cols <- setdiff(dx_cols, "dx_pscat")  # Exclude if necessary

long_dx <- clinical_diagnosis_df %>%
  select(bblid, all_of(dx_cols)) %>%
  pivot_longer(cols = all_of(dx_cols), names_to = "diagnosis", values_to = "has_dx") %>%
  filter(has_dx == 1)

target_flows <- long_dx %>%
  group_by(target_label = diagnosis) %>%
  summarise(unique_participants = n_distinct(bblid), .groups = "drop") %>%
  mutate(
    target_pct = round(100 * unique_participants / total_participants, 1)
  )

# Step 5: Apply readable labels to diagnosis names
diagnosis_labels <- c(
  dx_adhd = "ADHD",
  dx_adhd_remit = "ADHD (Remitted)",
  dx_bp = "BPD",
  dx_bp_remit = "BPD (Remitted)",
  dx_mdd = "MDD",
  dx_mdd_remit = "MDD (Remitted)",
  dx_moodnos = "Mood Disorder NOS",
  dx_moodnos_remit = "Mood Disorder NOS (Remitted)",
  dx_none = "No Diagnosis",
  dx_other = "Other",
  dx_other_remit = "Other (Remitted)",
  dx_prodromal = "CHR/Prodromal Psychosis",
  dx_prodromal_remit = "CHR/Prodromal Psychosis (Remitted)",
  dx_psychosis = "Psychosis",
  dx_psychosis_remit = "Psychosis (Remitted)",
  dx_scz = "Schizophrenia",
  dx_scz_remit = "Schizophrenia (Remitted)",
  dx_ptsd = "PTSD",
  dx_ptsd_remit = "PTSD (Remitted)"
)

target_flows <- target_flows %>%
  mutate(target_label = recode(target_label, !!!diagnosis_labels)) %>%
  mutate(target_label_pct = paste0(target_label, " (", target_pct, "%)"))

# Step 6: Join percent labels back to main summary
summary_long_labeled <- summary_long_no0 %>%
  left_join(source_flows, by = "source_label") %>%
  left_join(target_flows, by = "target_label")

# Step 7: Create nodes and links
nodes <- data.frame(
  name = unique(c(summary_long_labeled$source_label_pct,
                  summary_long_labeled$target_label_pct)),
  stringsAsFactors = FALSE
)

links <- summary_long_labeled %>%
  mutate(
    source = match(source_label_pct, nodes$name) - 1,
    target = match(target_label_pct, nodes$name) - 1
  ) %>%
  select(source, target, value = count)

# Step 8: Plot Sankey (identical), then hide in-graph labels
sankey_plot <- sankeyNetwork(
  Links = links,
  Nodes = nodes,
  Source = "source",
  Target = "target",
  Value  = "value",
  NodeID = "name",
  fontSize = 12,
  nodeWidth = 30
)

sankey_plot <- htmlwidgets::onRender(
  sankey_plot,
  jsCode = "
  function(el, x) {
    d3.select(el).selectAll('.node text').text(''); // blank node labels
  }
  "
)

# Step 9: Save Sankey HTML and PNG (same workflow as your original)
out_html <- "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_blank.html"
out_png  <- "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_blank.png"

htmlwidgets::saveWidget(
  sankey_plot,
  file = out_html,
  selfcontained = TRUE
)

webshot2::webshot(
  url     = out_html,
  file    = out_png,
  vwidth  = 1000,
  vheight = 500,
  zoom    = 3.83
)


```









```{r sankey plot with percentages - no labels OLD?WRONG?}

# =========================
# Libraries
# =========================
library(dplyr)
library(tidyr)
library(networkD3)
library(htmlwidgets)
library(htmltools)

# =========================
# Data prep (as in your code)
# =========================

# Ensure all TD/NC participants have dx_none == 1
clinical_diagnosis_df <- clinical_diagnosis_df %>%
  mutate(dx_none = ifelse(study_group == "TD/NC", 1, dx_none))

# Clean source/target labels from your summary table
summary_long_no0 <- summary_long_no0 %>%
  mutate(
    source_label = study_group,
    target_label = diagnosis
  )

# Total unique participants
total_participants <- clinical_diagnosis_df %>%
  distinct(bblid) %>%
  nrow()

# % per study group (left)
source_flows <- clinical_diagnosis_df %>%
  group_by(source_label = study_group) %>%
  summarise(unique_participants = n_distinct(bblid), .groups = "drop") %>%
  mutate(
    source_pct = round(100 * unique_participants / total_participants, 1),
    source_label_pct = paste0(source_label, " (", source_pct, "%)")
  )

# % per diagnosis (right)
dx_cols <- grep("^dx_", names(clinical_diagnosis_df), value = TRUE)
dx_cols <- setdiff(dx_cols, "dx_pscat")  # Exclude if necessary

long_dx <- clinical_diagnosis_df %>%
  select(bblid, all_of(dx_cols)) %>%
  pivot_longer(cols = all_of(dx_cols), names_to = "diagnosis", values_to = "has_dx") %>%
  filter(has_dx == 1)

target_flows <- long_dx %>%
  group_by(target_label = diagnosis) %>%
  summarise(unique_participants = n_distinct(bblid), .groups = "drop") %>%
  mutate(
    target_pct = round(100 * unique_participants / total_participants, 1)
  )

# Readable diagnosis labels
diagnosis_labels <- c(
  dx_adhd = "ADHD",
  dx_adhd_remit = "ADHD (Remitted)",
  dx_bp = "BPD",
  dx_bp_remit = "BPD (Remitted)",
  dx_mdd = "MDD",
  dx_mdd_remit = "MDD (Remitted)",
  dx_moodnos = "Mood Disorder NOS",
  dx_moodnos_remit = "Mood Disorder NOS (Remitted)",
  dx_none = "No Diagnosis",
  dx_other = "Other",
  dx_other_remit = "Other (Remitted)",
  dx_prodromal = "CHR/Prodromal Psychosis",
  dx_prodromal_remit = "CHR/Prodromal Psychosis (Remitted)",
  dx_psychosis = "Psychosis",
  dx_psychosis_remit = "Psychosis (Remitted)",
  dx_scz = "Schizophrenia",
  dx_scz_remit = "Schizophrenia (Remitted)",
  dx_ptsd = "PTSD",
  dx_ptsd_remit = "PTSD (Remitted)"
)

target_flows <- target_flows %>%
  mutate(target_label = recode(target_label, !!!diagnosis_labels)) %>%
  mutate(target_label_pct = paste0(target_label, " (", target_pct, "%)"))

# Join % labels back to main summary
summary_long_labeled <- summary_long_no0 %>%
  left_join(source_flows, by = "source_label") %>%
  left_join(target_flows, by = "target_label")

# =========================
# Nodes & Links (plain data.frames)
# =========================
nodes <- data.frame(
  name = unique(c(
    as.character(summary_long_labeled$source_label_pct),
    as.character(summary_long_labeled$target_label_pct)
  )),
  stringsAsFactors = FALSE, check.names = FALSE
)
nodes <- nodes[!is.na(nodes$name) & nodes$name != "", , drop = FALSE]

links <- summary_long_labeled %>%
  mutate(
    source = match(source_label_pct, nodes$name) - 1,
    target = match(target_label_pct, nodes$name) - 1
  ) %>%
  select(source, target, value = count) %>%
  as.data.frame()

# =========================
# Color mapping (matches your screenshot)
# =========================
# Base label (strip " (..%)")
nodes$base <- sub(" \\([^)]+\\)$", "", nodes$name)

# Palette keyed by base labels
group_palette <- c(
  "ADHD"                     = "#1f77b4",
  "PRO/CHR"                  = "#ff7f0e",
  "TD/NC"                    = "#aec7e8",
  "CHR/Prodromal Psychosis"  = "#e377c2",
  "Psychosis"                = "#9467bd",
  "MDD"                      = "#c5b0d5",
  "Mood Disorder NOS"        = "#fdae6b",
  "BPD"                      = "#2ca02c",
  "PTSD"                     = "#98df8a",
  "No Diagnosis"             = "#d62728"
)

nodes$group <- nodes$base
used_groups <- intersect(unique(nodes$group), names(group_palette))

colourScale_js <- htmlwidgets::JS(
  sprintf(
    'd3.scaleOrdinal().domain(%s).range(%s)',
    jsonlite::toJSON(unbox = FALSE, used_groups),
    jsonlite::toJSON(unbox = FALSE, unname(group_palette[used_groups]))
  )
)

# =========================
# Sankey (NO on-graph labels)
# =========================
sankey_plot <- sankeyNetwork(
  Links       = links,
  Nodes       = nodes,
  Source      = "source",
  Target      = "target",
  Value       = "value",
  NodeID      = "name",   # keep NodeID for widget internals…
  fontSize    = 0,        # …but hide the labels
  nodeWidth   = 30,
  nodePadding = 20,
  colourScale = colourScale_js
)

# Extra safety: hide any node text via D3
sankey_plot <- htmlwidgets::onRender(
  sankey_plot,
  htmlwidgets::JS("function(el){ d3.select(el).selectAll('.node text').style('display','none'); }")
)

# ---- Legend (left/right, with exact colors and %s) ----
left_items <- source_flows %>%
  distinct(base = source_label, label = source_label_pct) %>%
  filter(!is.na(base), base != "")

right_items <- target_flows %>%
  distinct(base = target_label, label = target_label_pct) %>%
  filter(!is.na(base), base != "")

safe_color <- function(key) {
  if (is.null(key) || length(key) == 0 || is.na(key) || !(key %in% names(group_palette))) {
    "#999999"  # fallback color
  } else {
    group_palette[[key]]
  }
}

make_li <- function(text, base_key) {
  col <- safe_color(base_key)
  htmltools::tags$li(
    htmltools::tags$span(style = sprintf(
      "display:inline-block;width:14px;height:14px;background:%s;margin-right:8px;vertical-align:middle;border-radius:3px;",
      col
    )),
    htmltools::tags$span(text)
  )
}

legend_html <- htmltools::tags$div(
  style = "margin-top: 18px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;",
  htmltools::tags$div(
    style = "display:flex; gap:48px; align-items:flex-start; flex-wrap:wrap;",
    htmltools::tags$div(
      htmltools::tags$h4("Study Group (Left)"),
      htmltools::tags$ul(style="list-style:none; padding-left:0; margin-top:8px;",
        lapply(seq_len(nrow(left_items)), function(i) make_li(left_items$label[i], left_items$base[i])))
    ),
    htmltools::tags$div(
      htmltools::tags$h4("Diagnosis (Right)"),
      htmltools::tags$ul(style="list-style:none; padding-left:0; margin-top:8px;",
        lapply(seq_len(nrow(right_items)), function(i) make_li(right_items$label[i], right_items$base[i])))
    )
  )
)

# >>> KEY CHANGE: append legend to the *widget* (no browsable/tagList) <<<
widget_with_legend <- htmlwidgets::appendContent(sankey_plot, legend_html)

# ---- Save HTML (non-selfcontained) and then PNG via file:// ----
out_html <- "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_legend.html"
out_png  <- "/Users/bsevchik/projects/EF_clinical/final_clinical_CNB/clinical/diagnosis_sankey_plot_legend.png"

htmlwidgets::saveWidget(
  widget = widget_with_legend,
  file   = out_html,
  selfcontained = FALSE   # more reliable for complex widgets
)

# Use a temporary HTML for the snapshot to avoid asset path surprises
tmp_html <- file.path(tempdir(), "sankey_legend_tmp.html")
htmlwidgets::saveWidget(
  widget = widget_with_legend,
  file   = tmp_html,
  selfcontained = FALSE
)

webshot2::webshot(
  url     = paste0("file://", normalizePath(tmp_html)),
  file    = out_png,
  vwidth  = 1000,
  vheight = 650,
  zoom    = 3.83
)


```

