...
# Define the fields to add for ASL and M0 scan JSON files
asl_fields = {
    "RepetitionTimePreparation": 4.29845,
...
def update_json_sidecar(file_path, fields):
    try:
        # Open and load the existing JSON sidecar
        with open(file_path, 'r') as json_file:
            json_data = json.load(json_file)
        
        # Add the fields to the JSON data
...
# Loop through the subjects and sessions to find ASL and M0 scan files
                ...
                if os.path.isdir(perf_path):
                    for file_name in os.listdir(perf_path):
                        ...
                        if file_name.endswith('asl.json'):
                            # Update ASL JSON sidecar
                            update_json_sidecar(file_path, asl_fields)
                        elif file_name.endswith('m0scan.json'):
                            # Update M0 scan JSON sidecar
                            update_json_sidecar(file_path, m0scan_fields)
