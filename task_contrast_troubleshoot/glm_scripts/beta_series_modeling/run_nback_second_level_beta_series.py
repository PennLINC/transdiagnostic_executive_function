#!/usr/bin/env python
"""Run second-level GLM for n-back task across all subjects, including LSA block contrasts."""
...
# Averaged condition contrasts plus individual LSA block-vs-baseline contrasts.
# These labels should match the contrast labels written by the first-level LSA script.
CONTRAST_LABELS = [
    "twoBack",
    "zeroBack",
...
firstlevel_dir = Path(
    ...
    "results_beta_series/first-level/nback-block-lsa"
)
...
for contrast_label in CONTRAST_LABELS:
    ...
    group_out_dir = Path(
        "/cbica/projects/executive_function/code/task_contrast/final/"
        f"results_beta_series/second-level/nback-block-lsa/group-{contrast_label}"
    )
    group_out_dir.mkdir(exist_ok=True, parents=True)

    print("MODEL TYPE: LSA block")
